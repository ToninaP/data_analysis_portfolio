...
def plot_gender_pyramid_creation(museum_data, museum_names, min_year=1860):
    # prepare data
    ...
    for df in valid_museum_data:
        ...
        df["Age_Group"] = pd.cut(
            df["Artist_age_creation"], bins=bins, labels=labels, right=True
        )
    ...
    fig.update_layout(
        title="Gender pyramid in collections: Artists at the artwork creation age",
